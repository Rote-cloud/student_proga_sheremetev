source_system = input().split(" ")
n = int(source_system[0])
source_system = [int(source_system[i]) for i in range(len(source_system)) if i != 0]
...
for i in range(len(count_origin) - 1):
    ...
    count_origin[i + 1] += (count_origin[i] - take_away_arr[i]) * source_system[i]
