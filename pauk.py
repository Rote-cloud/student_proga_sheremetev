...
for i in range(len(s)):
    ...
    if koor[i] == max(s[i], f[i]) and min(s[i], f[i]) == 0:
        answer += round(min(koordin(s[(i + 1) % 3] + koor[(i + 1) % 3] + f[(i + 1) % 3], koor_rez), koordin(2 * koor[i] - f[i] - s[i] + koor[(i + 1) % 3], koor_rez)), 3)
        break
    elif ((koor[i] == s[i] and koor[i] == f[i]) or (s[i] == 0 and f[i] == 0)):
        answer += round(koordin(s[(i + 1) % 3] - f[(i + 1) % 3], koor_rez), 3)
        break
    else:
        if s[i] not in [0, koor[i]] and f[i] not in [0, koor[i]] or (s[i] - f[i]) == 0:
